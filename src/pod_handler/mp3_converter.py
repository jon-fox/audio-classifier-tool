...
def find_ad_timestamps(transcript):
    ...
    for t_segment in transcript:
        ...
        contains_ad = any(company in text for company in ad_companies) or any(
...
def extract_segments(segments, start_time, end_time):
    ...
    extracted_segments = [
        segment for segment in segments if start_time <= segment["start"] <= end_time
    ]
...
def process_audio_segment(index, audio_segment):
    ...
    if min_ms == float("inf") and max_ms == float("-inf"):
        ...
        if start_ads_ms < 0:
            logger.info(
                "transcript_{index}_logging.json Start time is less than 0 or None, setting to 0",
                start_ads_ms,
            )
            start_ads_ms = 0
        if end_ads_ms > len(audio_segment):
            logger.info(
                f"transcript_{index}_logging.json End time is greater than segment duration or None, setting to segment duration",
                end_ads_ms,
            )
...
def remove_ads_from_audio(audio_file):
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        future_to_segment = {
            executor.submit(process_audio_segment, i, segments[i]): i
